...
2. Build and test the application and updater locally.
3. Commit and push all release changes to `main`.
4. Create and push a matching annotated tag, for example `v1.9.0`.
5. The `Build and publish release` GitHub Actions workflow builds the projects, creates the Windows ZIP and SHA-256 file, and publishes the GitHub Release.
6. Confirm that the release is not marked as a draft or prerelease and that both required assets are present.
7. Test **Check for Updates** from the previous released version before announcing the release.
