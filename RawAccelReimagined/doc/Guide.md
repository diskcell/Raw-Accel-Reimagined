...
Visit the [Raw Accel Reimagined Releases page](https://github.com/diskcell/Raw-Accel-Reimagined/releases) and download `Raw-Accel-Reimagined-Windows-x64.zip` from the latest stable release. **DO NOT** use GitHub's green "Code"/"Download ZIP" button, because that downloads the source tree instead of the prepared Windows application package.
...
- Run `RawAccelReimagined.exe` from the extracted application folder after the driver is installed.
